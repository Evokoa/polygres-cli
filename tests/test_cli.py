...
def test_client_retry_wait_is_clamped_to_command_deadline(
    ...
    sleeps: list[float] = []
    monkeypatch.setattr("polygres_cli.cli_client.time.monotonic", lambda: 10.0)
...
def test_quiet_login_prints_essential_fallback_to_stderr_when_browser_fails(
    ...
    assert "ABCD-EFGH" not in err
    assert POLL_TOKEN not in err
...
def test_login_start_response_requires_a_nonempty_poll_token(payload: dict[str, object]) -> None:
...
def test_poll_token_is_redacted_from_nested_cli_payloads() -> None:
    assert redact({"poll_token": POLL_TOKEN, "nested": [POLL_TOKEN]}) == {
        "poll_token": "[REDACTED]",
        "nested": ["[REDACTED]"],
    }
